...
const getResumenSemanal = async (req, res) => {
    ...
    try {
        ...
        // 🔥 Aislar suma de cierres por tienda
        let queryCierres = `
        ...
        `;
        if (!esUsuarioMaestro) queryCierres += ` AND tienda_id = ${idTiendaLocal}`;
        queryCierres += ` ORDER BY fecha_cierre ASC`;

        const cierresRes = await pool.query(queryCierres, [fechaInicio, fechaFin]);
        ...
        res.json({
            ...
            cumple_seis_dias: cantidadDias === 6,
            totales: { total_usd: granTotalUSD.toFixed(2), total_bs: granTotalBs.toFixed(2) },
...
const exportarExcelSemanal = async (req, res) => {
    ...
    try {
        ...
        const cierresRes = await pool.query(queryCierres, [fecha_inicio, fecha_fin]);
        ...
        const worksheet = workbook.addWorksheet('Auditoría Semanal', { views: [{ showGridLines: true }] });

        worksheet.columns = [
            { key: 'metodo', width: 30 }, { key: 'transacciones', width: 20 },
            { key: 'total_usd', width: 25 }, { key: 'total_bs', width: 25 }
        ];
        ...
        rangoCell.value = `PERÍODO: Desde el Lunes ${fInicioClean} hasta el Sábado ${fFinClean} ${!esUsuarioMaestro ? '- Sucursal ' + idTiendaLocal : ''}`;
        rangoCell.font = { name: 'Arial', size: 10, bold: true, italic: true, color: { argb: 'FF475569' } };
        worksheet.addRow([]); 

        const headerRow = worksheet.addRow(['MÉTODO DE PAGO', 'CANT. OPERACIONES', 'VOLUMEN TOTAL (USD)', 'VOLUMEN TOTAL (BS)']);
        headerRow.height = 28;
        headerRow.eachCell((cell) => {
            cell.font = { name: 'Arial', bold: true, color: { argb: 'FFFFFFFF' }, size: 10 };
            cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1E293B' } }; 
            cell.alignment = { vertical: 'middle', horizontal: 'center' };
            cell.border = { bottom: { style: 'medium', color: { argb: 'FF0F172A' } }, top: { style: 'thin', color: { argb: 'FF1E293B' } } };
        });

        const borderFino = { bottom: { style: 'thin', color: { argb: 'FFE2E8F0' } }, left: { style: 'thin', color: { argb: 'FFE2E8F0' } }, right: { style: 'thin', color: { argb: 'FFE2E8F0' } } };

        Object.values(resumenMap).forEach((row, index) => {
            const newRow = worksheet.addRow([row.metodo, row.transacciones, row.total_usd, row.total_bs]);
            newRow.height = 22;
            const fondoFila = (index % 2 === 1) ? 'FFF8FAFC' : 'FFFFFFFF'; 

            newRow.eachCell((cell, colNum) => {
                cell.font = { name: 'Arial', size: 10, color: { argb: 'FF334155' } };
                cell.border = borderFino;
                cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: fondoFila } };
                if (colNum === 1) { cell.alignment = { horizontal: 'left', vertical: 'middle' }; cell.font = { name: 'Arial', size: 10, bold: true, color: { argb: 'FF0F172A' } }; }
                else if (colNum === 2) { cell.alignment = { horizontal: 'center', vertical: 'middle' }; cell.numFmt = '#,##0'; }
                else if (colNum === 3) { cell.alignment = { horizontal: 'right', vertical: 'middle' }; cell.numFmt = '$#,##0.00'; }
                else if (colNum === 4) { cell.alignment = { horizontal: 'right', vertical: 'middle' }; cell.numFmt = '"Bs. "#,##0.00'; }
            });
        });

        const rowTotal = worksheet.addRow(['TOTALES GENERALES', totalGeneralTransacciones, totalGeneralUSD, totalGeneralBs]);
        rowTotal.height = 26;
        rowTotal.eachCell((cell, colNum) => {
            cell.font = { name: 'Arial', bold: true, size: 11, color: { argb: 'FF0F172A' } };
            cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF1F5F9' } };             
            cell.border = { top: { style: 'thin', color: { argb: 'FF94A3B8' } }, bottom: { style: 'double', color: { argb: 'FF0F172A' } }, left: { style: 'thin', color: { argb: 'FFE2E8F0' } }, right: { style: 'thin', color: { argb: 'FFE2E8F0' } } };
