...
const consumirFormulaExterna = async (req, res) => {
    ...
    try {
        ...
        for (const lote of lotes) {
            ...
            const fijadorExtra = parseFloat(lote.fijadorExtra) || 0; // LECTURA DEL NUEVO EXTRA
            ...
            const esenciaRes = await client.query(`SELECT nombre, categoria FROM productos WHERE id = $1`, [productoId]);
            if (esenciaRes.rows.length === 0) throw new Error("La esencia seleccionada no existe.");
            ...
            const alcoholRes = await client.query(`SELECT id, nombre, categoria FROM productos WHERE categoria ILIKE '%alcohol%' AND activo=true LIMIT 1`);
            const fijadorRes = await client.query(`SELECT id, nombre, categoria FROM productos WHERE categoria ILIKE '%fijador%' AND activo=true LIMIT 1`);
            ...
            if (cantAlcohol > 0) {
                if (alcoholRes.rows.length > 0) {
                    ...
                    throw new Error(`Falta Alcohol: La receta requiere ${cantAlcohol}ml, pero no se encontró en inventario.`);
                }
            }

            // 3. DESCONTAR FIJADOR (Fórmula Base + FIJADOR EXTRA)
            const cantFijador = parseFloat(f.gramos_fijador) || 0;
            const fijadorTotal = (cantFijador + fijadorExtra) * cantVendida; // MATEMÁTICA EXACTA DEL FIJADOR
            ...
            if (fijadorTotal > 0) {
                if (fijadorRes.rows.length > 0) {
                    ...
                    throw new Error(`Falta Fijador: La receta requiere ${fijadorTotal}g, pero no se encontró en inventario.`);
                }
            }
            ...
            if (!esRecarga) {
                ...
                if (frascoRes.rows.length > 0) {
                    ...
                    throw new Error(`No se encontró stock de frascos de ${volumen}ml necesarios para la medida "${f.nombre}".`);
                }
            }
        }
        ...
        // =========================================================================
        // 🧠 MOTOR DE VALIDACIÓN DE STOCK Y AUTO-COMPOSICIÓN
        // =========================================================================
        for (const id in consolidado) {
            const item = consolidado[id];
            const stockRes = await client.query(`SELECT stock_unidades, stock_estante FROM productos WHERE id = $1`, [id]);
            if (stockRes.rows.length === 0) throw new Error(`El producto con ID ${id} no existe.`);
            ...
            // REGLA DE ORO: Si ni sumando ambos inventarios alcanza, quiebre absoluto (Evita negativos)
            if (totalGlobal < item.requerido) {
                throw new Error(`Quiebre de existencias absoluto: "${item.nombre}" requiere ${item.requerido} en total, pero solo hay ${totalGlobal} en todo el sistema. Operación rechazada.`);
            }

            // Si el origen solicitado es ESTANTE pero el mostrador no da la talla:
            if (origen === 'ESTANTE' && estante < item.requerido) {
                const faltante = item.requerido - estante;
                
                // Registramos para activar la extracción controlada de la bodega
        ...
        // =========================================================================
        // ⚡ EJECUCIÓN DEL DESCUENTO CON COMPENSACIÓN CRUZA-ZONAS
        // =========================================================================
        for (const id in consolidado) {
            const item = consolidado[id];
            ...
            // Verificar si este artículo específico necesita auxilio del Almacén General
            const autoCompo = alertasAutoComposicion.find(a => a.productoId === id);

            if (autoCompo) {
                fueAutoCompuesto = true;
                const stockRes = await client.query(`SELECT stock_estante FROM productos WHERE id = $1`, [id]);
                const estanteActual = parseFloat(stockRes.rows[0].stock_estante) || 0;

                // 1. Raspamos y dejamos en 0 lo que quedaba en mostrador
                if (estanteActual > 0) {
                    restadoEstante = estanteActual;
                    await client.query(`UPDATE productos SET stock_estante = 0 WHERE id = $1`, [id]);
                    await descontarDeBotellasEstante(id, estanteActual);
                }

                // 2. El déficit se cobra directamente a las cajas del Almacén
                restadoAlmacen = autoCompo.tomarAlmacen;
                await client.query(`UPDATE productos SET stock_unidades = stock_unidades - $1 WHERE id = $2`, [autoCompo.tomarAlmacen, id]);

                // 3. Asentamos la auditoría en la tabla dedicada
                ...
                // FLUJO ESTÁNDAR DE RETROCOMPATIBILIDAD
                if (origen === 'ESTANTE') {
                    restadoEstante = restante;
                    await client.query(`UPDATE productos SET stock_estante = stock_estante - $1 WHERE id = $2`, [restante, id]);
                    await descontarDeBotellasEstante(id, restante);
                } else {
                    const stockRes = await client.query(`SELECT stock_unidades FROM productos WHERE id = $1`, [id]);
                    let almacenActual = parseFloat(stockRes.rows[0].stock_unidades) || 0;

                    if (almacenActual > 0) {
                        if (almacenActual >= restante) {
                            await client.query(`UPDATE productos SET stock_unidades = stock_unidades - $1 WHERE id = $2`, [restante, id]);
                            restadoAlmacen = restante;
                            restante = 0;
                        } else {
                            restante -= almacenActual;
                            restadoAlmacen = almacenActual;
                            await client.query(`UPDATE productos SET stock_unidades = 0 WHERE id = $1`, [id]);
                        }
                    }
            ...
            await client.query(`
                ...
                VALUES ($1, 'SALIDA', $2, (SELECT stock_unidades + stock_estante FROM productos WHERE id=$1), $3, NOW())
            `, [id, item.requerido, motivoDetallado]);
        }

        await client.query(`
            INSERT INTO historial_sincronizacion (usuario_id, cantidad_items, detalles_json)
            VALUES ($1, $2, $3)
        `, [usuarioId, lotes.length, JSON.stringify(reporteMovimientos)]);
...
// =========================================================================
// NUEVA FUNCIÓN: Obtener el historial de descargas externas agrupadas
// =========================================================================
